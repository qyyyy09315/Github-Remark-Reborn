# GitHub Remark - Tampermonkey

[![](https://img.shields.io/badge/dynamic/json?color=green&label=version&query=version&url=https%3A%2F%2Fgreasyfork.org%2Fscripts%2F443857.json&logo=data:image/png;base64,)](https://greasyfork.org/zh-CN/scripts/443857-github-remark)

> 这是 [GithubRemark](https://github.com/dpy1123/GithubRemark) 的 Tampermonkey 本地版本。

给 GitHub 上的任意用户添加自定义备注名，类似微信备注功能。

---

## 安装

1. 浏览器安装 [Tampermonkey](https://www.tampermonkey.net/) 扩展
2. 点击上方版本号徽章前往 Greasy Fork 安装，或将 `GitHubRemark.js` 手动导入 Tampermonkey
3. 刷新 GitHub 页面即可使用

---

## 使用

- 所有 GitHub 用户名后会显示默认备注 `(unset)`
- **双击**备注文字即可修改，弹出输入框输入新备注
- 备注数据存储在浏览器 `localStorage` 中，无需联网
- 需要登录 GitHub 账号才能使用

---

## 支持页面

- 个人主页
- 首页动态流
- 关注 / 粉丝列表
- 仓库 Stargazers / Watchers
- 组织成员页

---

## 数据备份

所有备注使用 key `GithubRemark-cache` 存储在 `localStorage` 中，可通过浏览器开发者工具导出/导入。

---

## 相关版本

| 版本 | 说明 |
|------|------|
| [Chrome 扩展版](../) | Manifest V3，支持云端同步，适配新版 GitHub Turbo 框架 |
| Tampermonkey 版 | 纯本地版本，适合不需要云同步的轻量场景 |

---

## 作者

- **原作者** — [DD](https://github.com/dpy1123)
- **Tampermonkey 适配** — [Dorad](https://github.com/Doradx)
